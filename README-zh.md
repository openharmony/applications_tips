...
- 跨应用跳转：其他应用可通过显式Want启动`EntryAbility`，在参数中传入`detailLink`打开指定技巧详情。
...
| 跨应用打开指定技巧 | 其他应用通过Want传入`detailLink`，直接打开对应详情页。 |
...
3. 其他应用可通过显式Want启动`EntryAbility`，传入`detailLink`打开指定详情页。
...
本节仅列出跨应用打开玩机技巧详情页时需使用的Want参数，便于其他应用对接。
